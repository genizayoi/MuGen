...
export function getRandomInt(min: number, max: number, distribution: Distribution): number {
  const uniformRands = generate(()=>random.real(0, 1)).filter((n: number)=>0<n && n<1).memoize();
  switch (distribution) {
    case 'normal': {
      return Math.floor(Math.abs(uniformRands.first()) * (max - min)) + min;
    }
    case 'uniform': {
      const pseudoNormalRandomInt = uniformRands.first(10).reduce((a: number, b: number)=> a + b, 0) / 10;
